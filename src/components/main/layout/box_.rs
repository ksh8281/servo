...
impl ImageBoxInfo {
    ...
    pub fn image_width(&self) -> Au {
        let mut image_ref = self.image.borrow();
...
impl Box {
    ...
    pub fn assign_width(&self,container_width: Au) {
        match self.specific {
            ...
            ImageBox(ref image_box_info) => {
                let width = MaybeAuto::from_style(self.style().Box.width,container_width);
                let height = MaybeAuto::from_style(self.style().Box.height,container_width);
                ...
                self.position.borrow_mut().get().size.width = width;
